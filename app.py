...
def main():
    ...
    # Sidebar
    st.sidebar.image(
        "https://via.placeholder.com/200x80/1f77b4/ffffff?text=ProcureAI",
        width=200
    )
...
def show_home(df):
    ...
    with col4:
        if 'Annual_Procurement_Spend_AED' in df.columns:
            st.metric(
                "💼 Avg Procurement",
                f"AED {df['Annual_Procurement_Spend_AED'].mean()/1e6:.1f}M"
            )
    ...
    st.markdown(
        create_download_link(df, "procureai_data.csv", "📥 Download Current Dataset (CSV)"),
        unsafe_allow_html=True
    )
...
def show_classification(df):
    ...
    if 'Is_Hot_Lead' not in df.columns:
        if 'Interest_Level' in df.columns and 'Avg_Pain_Score' in df.columns:
            df['Is_Hot_Lead'] = (
                (df['Interest_Level'] >= 4) &
                (df['Avg_Pain_Score'] >= 3.5)
            ).astype(int)
        else:
            df['Is_Hot_Lead'] = 0
            st.warning(
                "⚠️ Creating dummy target variable. "
                "Upload data with Interest_Level and Avg_Pain_Score for better results."
            )
    ...
    numeric_features = [
        ...
        if f not in ['Is_Hot_Lead', 'Cluster', 'Predicted_Hot_Lead', 'Hot_Lead_Probability']
    ]

    default_features = [
        f for f in [
            'Avg_Pain_Score', 'Interest_Level', 'Purchase_Urgency',
    ...
    st.markdown(
        create_download_link(df, "classification_predictions.csv", "📥 Download All Predictions"),
        unsafe_allow_html=True
    )
...
def show_clustering(df):
    ...
    numeric_features = [
        f for f in numeric_features
        if f not in ['Cluster', 'Is_Hot_Lead', 'Predicted_Hot_Lead', 'Hot_Lead_Probability']
    ]

    default_features = [
        f for f in [
            'Annual_Procurement_Spend_AED', 'Max_Monthly_WTP_AED',
    ...
    with st.spinner("🔄 Computing elbow plot..."):
        ...
        fig = make_subplots(
            rows=1, cols=2,
            subplot_titles=('Elbow Plot', 'Silhouette Score')
        )
    ...
    # Build profile_features without duplicates
    raw_features = selected_features.copy()
    if 'Interest_Level' in df.columns:
        raw_features.append('Interest_Level')

    profile_features = []
    for f in raw_features:
        if f in df.columns and f not in profile_features:
            profile_features.append(f)
    ...
    # Extra safety: drop any duplicate columns if they somehow exist
    cluster_profiles = cluster_profiles.loc[:, ~cluster_profiles.columns.duplicated()]

    # Rename clusters with persona names
    ...
    if len(profile_features) == 0:
        st.warning("No numeric profile features available for comparison.")
        return
    ...
    st.markdown("---")
    st.markdown(
        create_download_link(df, "cluster_assignments.csv", "📥 Download Cluster Assignments"),
        unsafe_allow_html=True
    )
...
def show_association_rules(df):
    ...
    **Interpretation:**
    - {selected_rule['confidence']*100:.0f}% of customers who have {', '.join(list(selected_rule['antecedents']))}
      also have {', '.join(list(selected_rule['consequents']))}.  
    ...
    st.markdown(
        create_download_link(rules_export, "association_rules.csv", "📥 Download All Rules"),
        unsafe_allow_html=True
    )
...
def show_regression(df):
    ...
    default_features = [
        f for f in [
            'Annual_Procurement_Spend_AED', 'Avg_Pain_Score',
    ...
    st.markdown(
        create_download_link(df, "wtp_predictions.csv", "📥 Download All Predictions"),
        unsafe_allow_html=True
    )
...
def show_dynamic_pricing(df):
    ...
    if 'Predicted_WTP_AED' not in df.columns:
        ...
        if st.button("🔮 Generate Predictions Now"):
            ...
            numeric_features = [
                f for f in numeric_features
                if f not in ['Max_Monthly_WTP_AED', 'Cluster']
            ]
    ...
    with col2:
        ...
        fig = px.bar(
            x=tier_revenue.index, y=tier_revenue.values,
    ...
    tier_analysis.columns = [
        'Customer_Count', 'Avg_Price', 'Total_Revenue',
    ...
    st.markdown("---")
    st.markdown(
        create_download_link(df, "dynamic_pricing_recommendations.csv",
